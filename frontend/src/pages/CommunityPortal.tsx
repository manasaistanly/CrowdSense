    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-4xl mx-auto px-6 py-8 -mt-8">
                ...
                <div className="bg-white p-4 rounded-xl shadow-lg border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
                    <div className="flex items-center gap-3">
                        <MapPin className="h-6 w-6 text-primary-600" />
                        <div>
                            <span className="text-sm text-gray-500 block">Select Region</span>
                            <select
                                value={selectedDestinationId}
                                onChange={(e) => setSelectedDestinationId(e.target.value)}
                                className="font-bold text-gray-900 bg-transparent focus:outline-none text-lg cursor-pointer"
                            >
                                {destinations.map(d => (
                    ...
                    <div className="flex bg-gray-100 rounded-lg p-1">
                        <button
                            ...
                            className={`px-4 py-2 rounded-md text-sm font-medium transition ${activeTab === 'feedback' ? 'bg-white shadow text-primary-700' : 'text-gray-600 hover:text-gray-900'}`}
                        >
                            Feedback
                        </button>
                        <button
                            onClick={() => setActiveTab('report')}
                            className={`px-4 py-2 rounded-md text-sm font-medium transition ${activeTab === 'report' ? 'bg-white shadow text-red-700' : 'text-gray-600 hover:text-gray-900'}`}
                        >
                            Report Issue
                        </button>
                        <button
                            onClick={() => setActiveTab('transparency')}
                            className={`px-4 py-2 rounded-md text-sm font-medium transition ${activeTab === 'transparency' ? 'bg-white shadow text-primary-700' : 'text-gray-600 hover:text-gray-900'}`}
                        >
                            Transparency
                        </button>
                    </div>
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    ...
                    <div className="md:col-span-2 space-y-4">
                        <h2 className="font-bold text-gray-900 mb-2">Community Feed</h2>
                        {loading ? (
                            ...
                            <div className="text-center p-8 text-gray-500 bg-white rounded-xl">No active discussions yet. Be the first!</div>
                        ) : (
                                ...
                                <div key={item.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                                    <div className="flex justify-between items-start mb-3">
                                        <div className="flex items-center gap-2">
                                            {item.user.firstName === 'Anonymous' ? (
                                                <div className="bg-gray-200 p-2 rounded-full"><User className="h-4 w-4 text-gray-500" /></div>
                                            ) : (
                                                <div className="bg-primary-100 p-2 rounded-full text-primary-700 font-bold text-xs">
                                        ...
                                        <span className={`px-2 py-0.5 rounded text-xs font-bold ${item.feedbackType === 'INCIDENT_REPORT' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                                            }`}>
                                    ...
                                    <div className="flex items-center gap-4 pt-4 border-t border-gray-100">
                                        <button
                                            ...
                                            className="flex items-center gap-1.5 text-gray-500 hover:text-green-600 transition"
                                        >
